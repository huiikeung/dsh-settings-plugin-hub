 ...
 *   const SECTION_ID = "web-tools";  …  id: SECTION_ID,        ← 常量间接（dsh-search）
 *
 * 做法是逐个命中 slot 字面量，在其后 240 字符的窗口里取第一个 `id:`：
 *   - 值是字符串字面量 → 直接用；
 *   - 值是标识符（如 SECTION_ID）→ 回整份源码里找 `IDENT = "…"` 的第一次赋值。
...
export function extractSectionIds(source) {
  ...
  while ((match = slot.exec(text)) !== null) {
    ...
    const literalId = /(?:^|[{,\s])id\s*:\s*["']([^"']{1,120})["']/.exec(window);
    const identifierId = literalId === null
      ? /(?:^|[{,\s])id\s*:\s*([A-Za-z_$][A-Za-z0-9_$]{0,60})\b/.exec(window)
      : null;
    if (literalId === null && identifierId === null) continue;
    const at = literalId !== null ? literalId : identifierId;
    const between = window.slice(0, at.index);
    const others = between.match(/["']settings\.[a-z.]+["']/g) ?? [];
    if (others.some((literal) => !/settings\.section$/.test(literal.replace(/["']/g, '')))) continue;
    const id = literalId !== null ? literalId[1] : resolveConstant(text, identifierId[1]);
    if (id === null || id === SECTION_SLOT || id.length === 0) continue;
    ids.add(id);
  }
  return [...ids].sort();
}
...
/**
 * 在整份源码里解析 `IDENT = "…"` 的第一次赋值（const/var/let 均可）。
 * 只认「标识符 = 字符串字面量」这一种最简单形态；解析不出就返回 null，
 * 让该分页落进「来源未识别」，而不是猜一个值把别人的分页收走。
 * @param {string} text - 整份 bundle 源码。
 * @param {string} identifier - `id:` 处出现的标识符。
 * @returns {string|null}
 */
export function resolveConstant(text, identifier) {
  if (!/^[A-Za-z_$][A-Za-z0-9_$]{0,60}$/.test(identifier)) return null;
  const assignment = new RegExp(`\\b${identifier}\\s*=\\s*["']([^"']{1,120})["']`).exec(text);
  return assignment === null ? null : assignment[1];
}
