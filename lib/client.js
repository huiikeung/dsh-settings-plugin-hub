...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * （runtime 下留一堆 `.dsh-*.bak` 遗物就是证据）。
		 *
		 * 运行时按「导航格标签文本」找到自己的 cell，原地改写那个 <svg>：保留外壳给的
		 * 元素/类名/尺寸，只换几何，不依赖外壳的哈希类名。MutationObserver 应对外壳
		 * 重渲染导航时把齿轮换回来的情况。注意 applyRail() 只改 button 属性、不碰
		 * svg，所以两者互不干扰。
		 */
		function pinNavGlyph(labels, mark, glyph) {
			if (typeof document === "undefined" || typeof MutationObserver === "undefined") return;
			var applyGlyph = function () {
				// 便宜的前置判断：导航只在设置面板打开时才存在。
				if (document.querySelector('[role="dialog"]') === null) return;
				var cells = Array.from(document.querySelectorAll('[role="dialog"] nav button'));
				for (var i = 0; i < cells.length; i++) {
					...
					try {
						...
						console.warn("[dsh-settings-plugin-hub] nav glyph failed; keeping the shell icon", error);
						continue;
					}
		...
		/**
		 * 「第三方插件」分页的侧栏图标几何：IconCordisPluginOutline14（四向展开箭头，
		 * 自带 <defs><clipPath>）。数值从 DSH primitives 机械抽取并经 chromium 渲染验收，
		 * 不要手改；没有 stroke —— 形状全部 fill="currentColor"。
		 */
		function navGlyph() {
			return {
				viewBox: "0 0 14 14",
				markup: '<g clip-path="url(#clip0_1840_45990)">'
					+ '<path d="M3.03426 5.66661L1.70084 7.00003L3.0315 8.33069L2.14762 9.21457L-0.0669245 7.00003L2.15038 4.78273L3.03426 5.66661ZM7 14.067L4.77924 11.8462L5.66313 10.9623L7 12.2992L8.33342 10.9658L9.2173 11.8496L7 14.067ZM11.8489 9.21803L10.965 8.33414L12.2992 7.00003L10.9623 5.66316L11.8462 4.77927L14.0669 7.00003L11.8489 9.21803ZM8.33066 3.03153L7 1.70087L5.66589 3.03498L4.782 2.1511L7 -0.0668945L9.21454 2.14765L8.33066 3.03153Z" fill="currentColor"></path>'
					+ '<rect x="5.98535" y="5.98535" width="2.02942" height="2.02942" fill="currentColor"></rect>'
					+ '</g>'
					+ '<defs><clipPath id="clip0_1840_45990"><rect width="14" height="14" fill="currentColor"></rect></clipPath></defs>',
			};
		}
		...
		function apply(ctx) {
			installStyles();
			// 设置侧栏图标：契约没有 icon 字段，不 Pin 就是齿轮。从这里 Pin，DSH 升级/装卸都不丢。
			pinNavGlyph(["第三方插件"], "data-plugin-hub-nav-icon", navGlyph);
